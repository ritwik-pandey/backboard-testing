...
async def process_file_async(file_path):
    ...
    # 1. STRUCTURAL EXTRACTION (Pandas - CPU bound, runs sync)
    try:
        raw_df, header_idx = smart_read_statement(file_path)
        clean_df = normalize_columns(raw_df)
        
        # --- FIX: DATA TYPE CLEANING FOR JSON ---
        # Convert Dates to String (Fixes the JSON Error)
        if 'date' in clean_df.columns:
            clean_df['date'] = clean_df['date'].astype(str)
            
        # Ensure numbers are floats and remove commas
    ...
    # 2. INTELLIGENCE LAYER (Backboard - IO bound, runs async)
    # We await the AI response here
    print(f"🧠 Sending {filename} to Backboard AI...")
    
    # Pass the actual API Key if you have it, otherwise it uses the mock logic inside intelligence.py
